...
This example illustrates the case of a program with non linear expression
 */

class TritypeV8 {

	/*@ requires 
          @ (i==3 && j==4 && k==3)&& (2*(i+j+k)/2==(i+j+k));
	  @ ensures
	  @ (((((i+j) <= k || (j+k) <= i || (i+k) <= j) || (i==0 && j==0 && k==0)) ==> (\result == -1))
	  @  && (!(((i+j) <= k || (j+k) <= i || (i+k) <= j) || (i==0 && j==0 && k==0))) ==> (\result == (i+j+k)/2*((i+j+k)/2-i)*((i+j+k)/2-j)*((i+j+k)/2-k)));
	  @*/
	...
	static int caller (int i, int j, int k) {
	 ...
	 public static void main(String[] args) {
			TritypeV8 p = new TritypeV8();
			System.out.println(p.caller(3, 4, 3));		//20
		}
}
